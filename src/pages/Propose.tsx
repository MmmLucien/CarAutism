...
import { ALL_SUJETS, CATEGORIES } from '@/lib/constants'
...
// Build flat label map from CATEGORIES
...
export function Propose() {
  ...
  function canProceedToConfirm() {
    return (
      ...
      form.wrongAnswer1.trim().length >= 1 &&
      form.wrongAnswer2.trim().length >= 1
    )
  }
  ...
  async function submit() {
    ...
    const wrong = [form.wrongAnswer1, form.wrongAnswer2, form.wrongAnswer3]
      .map(w => w.trim())
      .filter(Boolean)
    ...
    try {
      ...
      if (err) {
        // Si la table n'existe pas encore, on fait semblant que ça marche
        if (err.code === '42P01') {
          setStep('done')
          return
        }
      ...
      // Fallback si Supabase pas encore configuré
      setStep('done')
    } finally {
      setLoading(false)
    }
      ...
      <div className="min-h-screen bg-brand-dark flex flex-col items-center justify-center p-6 gap-6 text-center">
        ...
        <div>
          ...
          <p className="text-brand-muted text-sm">
            Ta question a été soumise. Elle sera vérifiée avant d'être intégrée au quiz.
          </p>
        </div>
        ...
        <button onClick={() => navigate('/')} className="text-brand-muted text-sm underline">
          Retour à l'accueil
        </button>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-brand-dark flex flex-col">
      ...
      <div className="flex flex-col gap-4 p-5 pb-24 overflow-y-auto">

        {/* ── STEP 1 : Sujet ── */}
        {step === 'sujet' && (
          <>
            <p className="text-sm text-brand-muted">Choisis le sujet de ta question :</p>
            {CATEGORIES.map(cat => (
              <div key={cat.id}>
                <div className="text-[10px] text-brand-muted tracking-[2px] font-bold mb-2">
                  {cat.label}
                </div>
                  ...
                  {cat.sujets.map(s => (
                    <button
                      ...
                      className={`text-left rounded-xl px-4 py-3 border-2 transition-all text-sm
                        ${form.sujet === s.id
                          ? 'border-brand-red bg-[#1a0000] text-white'
                          : 'border-brand-line bg-brand-card text-brand-muted'}`}
                    >
                      <div className="font-black">{s.icon} {s.name}</div>
                      <div className="text-[10px] mt-0.5 opacity-60">{s.description}</div>
                    </button>
                  ))}
                </div>
              </div>
        ...
        {step === 'question' && (
          <>
            <div className="bg-brand-card rounded-xl px-4 py-2 text-sm">
            ...
            <div>
              <label className="text-xs text-brand-muted font-bold block mb-1">MAUVAISES RÉPONSES *</label>
              <div className="flex flex-col gap-2">
                {(['wrongAnswer1', 'wrongAnswer2', 'wrongAnswer3'] as const).map((field, i) => (
                  <input
                    key={field}
                    type="text"
                    value={form[field]}
                    onChange={e => set(field, e.target.value)}
                    placeholder={`Mauvaise réponse ${i + 1}${i === 2 ? ' (optionnel)' : ' *'}`}
                    className="input-field"
                    maxLength={100}
                  />
                ))}
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: C'était la dernière voiture de route de Maranello avant les années 90"
                className="input-field"
                maxLength={200}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: Wikipedia, livre, magazine…"
                className="input-field"
                maxLength={200}
              />
            </div>
        ...
        {step === 'confirm' && (
          <>
            <p className="text-sm text-brand-muted">Vérifie ta proposition avant d'envoyer :</p>
            ...
            <div className="card flex flex-col gap-3">
              ...
              <Row label="✗ Mauvaises réponses" val={[form.wrongAnswer1, form.wrongAnswer2, form.wrongAnswer3].filter(Boolean).join(' / ')} />
      ...
      {/* Bottom CTA */}
      {(step as string) !== 'done' && (
        <div className="fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto p-4 bg-brand-dark border-t border-brand-line">
          {step === 'sujet' && (
            <button
              onClick={() => setStep('question')}
              disabled={!canProceedToQuestion()}
